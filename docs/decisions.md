...
| **Inter in code and Figma; Roboto Mono stands in** (2026-09-23) | Code used to name no font, only a system stack, so the system had no typographic voice and Figma could only guess (SF Pro on a Mac, Inter in Figma). Inter is now self-hosted in `src/fonts/` (two woff2 files from Google Fonts, 134 KB, OFL) and named first in `font.sans`; the system stack stays behind it as the fallback. Files, not an npm package, so the no-dependencies rule holds. Mono is still a system stack in code with Roboto Mono in Figma, see `figma/GAPS.md` |
